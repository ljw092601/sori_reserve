...
export default function WeekGrid({
  ...
  function finishDrag() {
    ...
    router.push(`/reserve?date=${days[drag.day]}&start=${start}&end=${end}`);
  }

  return (
    <div
      className="select-none overflow-x-auto rounded-xl border border-zinc-200 bg-white"
      onMouseMove={(e) =>
    ...
    >
        ...
        <div className="grid grid-cols-[3rem_repeat(7,1fr)]">
          ...
          {days.map((day, i) => (
            ...
            >
              ...
              {/* 예약 블록 */}
              {blocksByDay[i].map(({ r, top, height }) => (
                <Link
                  key={r.id}
      ...
      <p className="border-t border-zinc-100 px-3 py-2 text-xs text-zinc-400">
        빈 시간을 드래그(또는 클릭)하면 그 시간으로 예약 화면이 열립니다. 예약
        블록을 누르면 상세/취소 화면으로 이동합니다.
      </p>
    </div>
  );
}
