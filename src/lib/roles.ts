...
// 서버 전용 (내부에서 supabaseAdmin 사용) — 클라이언트 컴포넌트에서 import 금지
 ...
 * fetchProfile이 cache()로 감싸여 있어 displayName과 같은 렌더 패스에서
 * 호출되면 쿼리를 공유한다.
 */
export async function getRole(
  ...
  if (profile?.role) return profile.role === "exec" ? "exec" : "member";
